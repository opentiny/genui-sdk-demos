...
> 默认聊天接口为 `http://localhost:3100/chat/completions`。
...
好了，现在生成式UI的能力已经集成到系统中，不过目前的生成式UI和系统还没有半点联系，是两个完全独立的模块。下面我们来将系统能力接入到生成式UI当中，使其成为一个智能导购助手。
...
新建一个文件 `src/genui/mcp/product-mcp.ts`。我们来开发一个商品搜索的mcp服务。
...
有了工具，还需要一个mcp-client去调用工具，下面我来编写mcp-client的代码
